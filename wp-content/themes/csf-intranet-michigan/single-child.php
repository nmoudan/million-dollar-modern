...
$args = array('child_of' => $id_cat, 'orderby'=>'id','order'=>'ASC');
$categories = get_categories( $args );
restore_current_blog();
if($categories[0]->term_id != '') {
	$term_child = $categories[0]->term_id;
}
else{
	$term_child = $id_cat;
}
$query = new WP_Query('cat='.$term_child.'&posts_per_page=1');
$post_ids = wp_list_pluck( $query->posts, 'ID' );
$url = get_the_permalink( $post_ids[0] );
if($url != ''){
	wp_redirect($url);
	exit();
}
...
<section id="three_bloc2" class="wpb_row  gray full-row">
	...
	<div class="wpb_wrapper">
		<div class="wpb_text_column wpb_content_element ">
			<div class="wpb_wrapper">
				<h1 style="text-align: center; text-transform: uppercase;"><strong><span style="color: #ffffff;"><?php bloginfo('name'); ?></span></strong></h1>
			</div>
		</div>
		<div class="vc_separator wpb_content_element vc_separator_align_center vc_sep_width_30 vc_sep_border_width_2 vc_sep_pos_align_center vc_separator_no_text vc_sep_color_orange" style="width: 15%;">
...
$queried_object = get_queried_object();
$id_cat = $queried_object->term_id;
$args = array('child_of' => $categories[0]->parent, 'orderby'=>'id','order'=>'ASC');
$categories = get_categories( $args );
restore_current_blog();
if($categories[0]->term_id != '') {
	$term_child = $categories[0]->term_id;
...
global $post, $post_id, $wp_query;
$queried_object = get_queried_object();
$id_post = $queried_object->ID;
$query = new WP_Query(array('post__in' => array($id_post))); 
}
...
<?php /*	<div class="about-author-sec">		  
