...
if ( $footer_show || is_archive() || is_single() || is_home() || is_search() || $is_buddy ) : ?>
...
<script type="text/javascript">
  ...
  jQuery('.list-items li a').each(function(e,i) {
    if (href.indexOf(jQuery(this).attr('href')) >= 0) {
      jQuery(this).addClass('taxonomy_menu_list_items');
    }
  });
});
</script>
-->
<span id="searchwplocation"></span>
<script type="text/javascript">
	jQuery(function() {
      var href = window.location.href;
      jQuery('.list-items li a').each(function(e,i) {
      ...
      //search css
      	jQuery('.search-form-icon').on('click', function(){
	        jQuery('ul#nav').addClass('remove-nav');
			jQuery( '.gsearch-form-box' ).addClass('show-sbox');
			jQuery('#gsearch-box').focus();
			jQuery('.searchwp-live-search-results').insertAfter('#gsearch-form-box');
		});
    	jQuery(document).on('click', function(ev){
    		var myID = ev.target.id;
    		if((myID !='gsearchbox-icon' ) && myID !='gsearch-box'){
    			jQuery( '#gsearch-form-box' ).removeClass('show-sbox');
    			jQuery('ul#nav').removeClass('remove-nav');
    			jQuery('.searchwp-live-search-results').insertBefore('#searchwplocation');
    		}	
    	});
    });
