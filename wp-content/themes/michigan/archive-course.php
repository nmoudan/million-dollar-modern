...
<div class="clearfix">
	...
	<?php 
	if ( is_search() ) {
		echo sprintf( __( 'Search Results: &ldquo;%s&rdquo;', 'lifterlms' ), get_search_query() );
	if ( get_query_var( 'paged' ) ) {
		echo sprintf( __( '&nbsp;&ndash; Page %s', 'lifterlms' ), get_query_var( 'paged' ) ); }
	} elseif ( is_tax() ) {
	...
	<main class="container content w-course-archive" role="main">
		...
		if ( have_posts() ){
						...
						<?php
							if (get_the_terms($post->ID, 'course_cat' )) {
								...
								foreach ( $categories as $category ) :
									...
									$typeName[] = '<a class="hcolorf" href="' . esc_url( get_category_link( $category ) ) . '" title="' . esc_attr( sprintf( __( 'View all courses under %s', 'michigan' ), $category->name ) ) . '">'. $cat_icon . esc_html( $category->name ). '</a>';
			...
			echo '<p class="lifterlms-info">'.esc_html_e( 'No products were found matching your selection.', 'michigan' ) .'</p>';
		} 
