...
<?php if (isset($lesson_features['date'])){ ?>
...
<div class="row">
	<div class="col-md-6">
		<div class="post-thumbnail quiz-thumbnail">
			<?php if(isset($lesson_features['image']) && has_post_thumbnail()){
	...
	<div class="col-md-6">
		...
		<div class="llms-template-wrapper">
			<h4 class="llms-content-block">
				<?php printf( __( '<i class="fa-asterisk"></i> Attempts Remaining: <span class="llms-content llms-attempts">%s</span>', 'lifterlms' ), $attempts_left ); ?>
			</h4>
		</div>
		<?php } //Time Limit
		if ( $time_limit ) { ?>
		...
		<div class="llms-template-wrapper">
			<h4 class="llms-content-block">
				<?php printf( __( '<i class="fa-clock-o"></i> Time Limit: <span class="llms-content">%s</span>', 'lifterlms' ), LLMS_Date::convert_to_hours_minutes_string( $time_limit ) ); ?>
			</h4>
		</div>
		<?php } //Timer
		if ($time_limit){ ?>
...
<?php
if(isset($lessone_features['comment'])){
	comments_template();
}
?>	
</section>
...
<div class="col-md-3 sidebar">	
	...
	<?php //back to course
	if(isset($lessone_features['instructor'])){
