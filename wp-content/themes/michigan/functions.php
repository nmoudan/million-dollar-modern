...
function michigan_webnus_course_price() {
	if(michigan_webnus_options::michigan_webnus_course_taking()){
		if(class_exists('LLMS_Product')){
			...
			$price = rwmb_meta( 'michigan_course_price_meta' );
			$price_text = (is_numeric($price))?$currency.$price:$price;
		}
...
function michigan_webnus_author_permalinks() {
    global $wp_rewrite;
	$author_permalink = michigan_webnus_options::michigan_webnus_author_permalink();
...
if ( ! function_exists ( 'michigan_webnus_topbar' ) ) {
...
function michigan_webnus_sidebar_init() {
	...
	if(function_exists('is_woocommerce')) {
        register_sidebar(array(
            ...
            'description' => esc_html__('This widget area should be used only for WooCommerce header cart widget', 'michigan' ),
            'before_widget' => '',
            'after_widget' => '',
            'before_title' => '',
            'after_title' => ''
...
// Custom Google Fonts
	wp_enqueue_style( 'custom-google-fonts', michigan_webnus_options::michigan_webnus_get_google_fonts() );
// Comment Reply JS
...
function michigan_webnus_custom_login_logo() {
    $logo = michigan_webnus_options::michigan_webnus_admin_login_logo();
...
function michigan_course_price_meta_data() {
	if( class_exists( 'LLMS_Product' ) ) :
		...
		$price 		   = ( get_post_meta( get_the_ID(), '_on_sale', true ) && $sale_price ) ? $sale_price : $regular_price;
	else :
		$price = rwmb_meta( 'michigan_course_price_meta' );
	endif;

	if ( $price > 0 ) :
		// delete_post_meta( get_the_ID(), 'michigan_course_free_meta' );
		add_post_meta( get_the_ID(), 'michigan_course_paid_meta', $price, true ) or update_post_meta( get_the_ID(), 'michigan_course_paid_meta', $price );
	else :
