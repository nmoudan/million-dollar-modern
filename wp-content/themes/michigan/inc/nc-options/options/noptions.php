...
class NHP_Options{
	...
	*/
	function __construct($sections = array(), $args = array(), $extra_tabs = array()){
		...
		$defaults['menu_title'] = esc_html__('Options', 'michigan');
		$defaults['page_icon'] = 'icon-themes';
		$defaults['page_title'] = esc_html__('Options', 'michigan');
		...
		$defaults['help_sidebar'] = esc_html__(' ', 'michigan');
	...
	*/
	function _enqueue(){
		...
		wp_localize_script('nhp-opts-js', 'nhp_opts', array('reset_confirm' => esc_html__('Are you sure? Resetting will loose all custom values.', 'michigan'), 'opt_name' => $this->args['opt_name']));
		...
		echo '<div class="wrap">';
			...
			echo '<form method="post" action="options.php" enctype="multipart/form-data" id="nhp-opts-form-wrapper">';
				...
				echo '<div id="nhp-opts-header">';
					...
					submit_button(esc_html__('Reset to Defaults', 'michigan'), 'secondary', $this->args['opt_name'].'[defaults]', false);
				...
				echo '</div>';
					...
					if(isset($_GET['settings-updated']) && $_GET['settings-updated'] == 'true' && get_transient('nhp-opts-saved') == '1'){
						if(isset($this->options['imported']) && $this->options['imported'] == 1){
							echo '<div id="nhp-opts-imported">'.apply_filters('nhp-opts-imported-text-'.$this->args['opt_name'], wp_kses( __('<strong>Settings Imported!</strong>', 'michigan'), array( 'strong' => array() ) ) ).'</div>';
						}else{
							echo '<div id="nhp-opts-save">'.apply_filters('nhp-opts-saved-text-'.$this->args['opt_name'], wp_kses( __('<strong>Settings Saved!</strong>', 'michigan'), array( 'strong' => array() ) ) ).'</div>';
						}
						delete_transient('nhp-opts-saved');
					}
					echo '<div id="nhp-opts-save-warn">'.apply_filters('nhp-opts-changed-text-'.$this->args['opt_name'], wp_kses( __('<strong>Settings have changed!, you should save them!</strong>', 'michigan'), array( 'strong' => array() ) ) ).'</div>';
					echo '<div id="nhp-opts-field-errors">'.wp_kses( __('<strong><span></span> error(s) were found!</strong>', 'michigan'), array( 'strong' => array(), 'span' => array() ) ).'</div>';
					
					echo '<div id="nhp-opts-field-warnings">'.wp_kses( __('<strong><span></span> warning(s) were found!</strong>', 'michigan'), array( 'strong' => array(), 'span' => array() ) ).'</div>';
					...
					echo '<ul id="nhp-opts-group-menu">';
						...
						if(true === $this->args['show_import_export']){
							echo '<li id="import_export_default_section_group_li" class="nhp-opts-group-tab-link-li">';
									echo '<a href="javascript:void(0);" id="import_export_default_section_group_li_a" class="nhp-opts-group-tab-link-a" data-rel="import_export_default"><img src="'.$this->url.'img/admin-import.png" /> <span>'.esc_html__('Import / Export', 'michigan').'</span></a>';
							echo '</li>';
						}//if
						...
						if(true === $this->args['dev_mode']){
							echo '<li id="dev_mode_default_section_group_li" class="nhp-opts-group-tab-link-li">';
									echo '<a href="javascript:void(0);" id="dev_mode_default_section_group_li_a" class="nhp-opts-group-tab-link-a custom-tab" data-rel="dev_mode_default"><img src="'.$this->url.'img/glyphicons/glyphicons_195_circle_info.png" /> <span>'.esc_html__('Dev Mode Info', 'michigan').'</span></a>';
							echo '</li>';
						}//if
				...
				echo '<div id="nhp-opts-main">';
					...
					if(true === $this->args['show_import_export']){
						echo '<div id="import_export_default_section_group'.'" class="nhp-opts-group-tab">';
							echo '<h3>'.esc_html__('Import / Export Options', 'michigan').'</h3>';
							
							echo '<h4>'.esc_html__('Import Options', 'michigan').'</h4>';
							...
							echo '<div id="nhp-opts-import-code-wrapper">';
								...
								echo '<div class="nhp-opts-section-desc">';
									...
									echo '<p class="description" id="import-code-description">'.apply_filters('nhp-opts-import-file-description',esc_html__('Input your backup file below and hit Import to restore your sites options from a backup.', 'michigan')).'</p>';
							...
							echo '<div id="nhp-opts-import-link-wrapper">';
								...
								echo '<div class="nhp-opts-section-desc">';
									...
									echo '<p class="description" id="import-link-description">'.apply_filters('nhp-opts-import-link-description',esc_html__('Select Template and hit Import to load the options.', 'michigan')).'</p>';
								
								echo '</div>';

								echo '<select id="import-link-value" name="'.$this->args['opt_name'].'[import_link]">
											<option value="'.get_template_directory_uri() . '/inc/theme-option-setings/online-learning.txt">Online Learning</option>
											<option value="'.get_template_directory_uri() . '/inc/theme-option-setings/kids.txt">Kids</option>
											<option value="'.get_template_directory_uri() . '/inc/theme-option-setings/high-school.txt">High School</option>
											<option value="'.get_template_directory_uri() . '/inc/theme-option-setings/college.txt">College</option>
										</select>';
							...
							echo '<p id="nhp-opts-import-action"><input type="submit" id="nhp-opts-import" name="'.$this->args['opt_name'].'[import]" class="button-primary" value="'.esc_html__('Import', 'michigan').'"> <span>'.apply_filters('nhp-opts-import-warning', esc_html__('WARNING! This will overwrite any existing options, please proceed with caution!', 'michigan')).'</span></p>';
							echo '<div id="import_divide"></div>';
							
							echo '<h4>'.esc_html__('Export Options', 'michigan').'</h4>';
							echo '<div class="nhp-opts-section-desc">';
					...
					if(true === $this->args['dev_mode']){
						echo '<div id="dev_mode_default_section_group'.'" class="nhp-opts-group-tab">';
							echo '<h3>'.esc_html__('Dev Mode Info', 'michigan').'</h3>';
							echo '<div class="nhp-opts-section-desc">';
