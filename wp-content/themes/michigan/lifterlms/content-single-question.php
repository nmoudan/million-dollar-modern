...
if ( ! $loop->have_posts() ) {
	...
	while ($loop->have_posts()) : $loop->the_post();
			...
			<div class="col-md-2 col-sm-2">
				...
				<span class="questions-total-number"> <?php printf( esc_html__( '%d of %d', 'lifterlms' ), ( empty( $current_question ) ? '' : $current_question ), $question_count ); ?> </span></div>
			</div>
			</div>
			<?php
				...
				<div class="col-md-8">
					...
					foreach ( $quiz->questions as $key => $value ) :
						if ( $value['id'] == $question->id ) :
							...
							?>
							<input id="llms_prev_question" type="submit" class="button" name="llms_prev_question" value="<?php esc_html_e( 'Previous Question', 'lifterlms' ); ?>" />
					...
					foreach ( $quiz->questions as $key => $value ) :
						if ( $value['id'] == $question->id ) :
							...
							if ( $next_question_key > ( count( $quiz->questions ) - 1 ) ) :
								$btn_text = esc_html__( 'Complete Quiz','lifterlms' );
							else :
								$btn_text = esc_html__( 'Next Question','lifterlms' );
							endif;
						endif;
					endforeach;
					?>
					<input id="llms_answer_question" type="submit" class="button" name="llms_answer_question" value="<?php printf( esc_html__( '%s', 'lifterlms' ), $btn_text ); ?>" />
