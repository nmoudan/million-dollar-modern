...
if ( get_query_var( 'product-id' ) ) {
	...
	llms_add_notice( esc_html__( 'Product not found.', 'lifterlms' ) );
}
...
if ($coupon_session) {
	...
	$info_message = sprintf( esc_html__( 'Coupon code "%s" has been applied to your order', 'lifterlms' ), $coupon_session->coupon_code );

	$savings = ( 'percent' == $coupon_session->type ) ? $coupon_session->amount.'%' : '$'.$coupon_session->amount;

	$info_message .= ' '.sprintf( esc_html__( '(%s off)', 'lifterlms' ), $savings );
} else {
	$info_message = apply_filters( 'lifterlms_checkout_coupon_message', esc_html__( 'Have a coupon?', 'lifterlms' ) );
	$info_message .= ' <a href="#" id="show-coupon">' . esc_html__( 'Click here to enter your code', 'lifterlms' ) . '</a>';
}
...
<form action="" method="post" id="llms-product-purchase-form">
	<div class="llms-checkout-wrapper">
		...
		<?php echo  '<h4>' .
			apply_filters( 'lifterlms_checkout_form_title', esc_html__( 'Confirm Purchase', 'lifterlms' ) ) . '</h4>'; ?>

		<!-- Product information -->
		<div class="llms-title-wrapper">
			<h4 class="llms-title"><?php echo($product)?'<a href="'.$product->guid.'">'.$product->post_title.'</a>':''; ?></h4>
		</div>
		...
		<div class="llms-coupon-entry llms-notice-box">
			<?php if ( $coupon_session ) : ?>
				<form class="llms-remove-coupon" id="llms-remove-coupon" method="post">
					<div class="llms-center-content">
						<input type="submit" class="llms-button-text" name="llms_remove_coupon" value="[<?php esc_html_e( 'Remove', 'lifterlms' ); ?>]" />
			...
			<form id="llms-checkout-coupon" method="post" style="display:none">
				<input type="text" name="coupon_code" class="llms-input-text" placeholder="<?php esc_html_e( 'Enter coupon code', 'lifterlms' ); ?>" id="coupon_code" value="" />
				<div class="llms-clear-box llms-center-content">
					<input type="submit" class="button llms-button" name="llms_apply_coupon" value="<?php esc_html_e( 'Apply Coupon', 'lifterlms' ); ?>" />
					<input type="hidden" name="product_id" value="<?php echo $product->ID; ?>" />
				</div>
				<div class="clear"></div>
		...
		<div class="llms-final-price-wrapper llms-clear-box">
			<h3 class="llms-price"><span class="llms-price-label"><?php esc_html_e( 'You Pay:', 'lifterlms' ); ?></span><span class="llms-final-price"></span></h3>
		</div>
		...
		<div class="llms-price-wrapper">
			<div class="llms-payment-methods llms-notice-box">
				...
				if ( $available_gateways = LLMS()->payment_gateways()->get_available_payment_gateways() ) {
					...
					echo '<p>' . esc_html__( 'Payment processing is currently disabled.', 'lifterlms' ) . '</p>';
				}
				?>
			</div>
			...
			<div class="llms-clear-box llms-center-content">
				...
				<input class="llms-button"
					...
					value="<?php esc_html_e( 'Buy Now', 'lifterlms' ); ?>" />
				<?php endif; ?>
			</div>
