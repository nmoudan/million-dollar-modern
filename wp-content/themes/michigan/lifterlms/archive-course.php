...
<div class="archive-course-wrap clearfix">
	...
	<?php 
	if ( is_search() ) {
		echo sprintf( esc_html__( 'Search Results: &ldquo;%s&rdquo;', 'lifterlms' ), get_search_query() );
	if ( get_query_var( 'paged' ) ) {
		echo sprintf( esc_html__( '&nbsp;&ndash; Page %s', 'lifterlms' ), get_query_var( 'paged' ) ); }
	} elseif ( is_tax() ) {
						...
						<div class="course-list-content">
							...
							if ( ! llms_is_user_enrolled( get_current_user_id(), $post->ID ) ){ ?>
								<div class="llms-price-wrapper">
									...
									echo ($course_price)?'<div class="course-list-price">'.$course_price.'</div>':'<div class="course-list-price">'.esc_html__('Free','michigan').'</div>';
									?>
								</div>
							<?php }
								...
								<div class="modern-content">
									...
									if ( ! llms_is_user_enrolled( get_current_user_id(), $post->ID ) ){ ?>
										<div class="llms-price-wrapper">
											...
											$price_text = $llms_product->get_single_price_html();
											$course_price = ($price_text)?$price_text:esc_html__('FREE','michigan');
											echo ($course_price)?'<h4 class="llms-price"><span>'.$course_price.'</span></h4>':'<h4 class="llms-price"><span>'.esc_html__('Free','michigan').'</span></h4>';
											?>
										</div>
									<?php } 
									?>
						...
						<div class="col-md-8 nopad-all none-grid">
							<div class="course-list-meta">
								<div class="clearfix">
									<?php 
									global $course;
									echo ($length_html = $course->get_lesson_length())?'<div class="col-md-2 col-sm-2 col-xs-6 nopad-all"><span class="course-list-duration"><i class="sl-clock"></i>'.$length_html.'</span></div>':'';
									$my_post = get_post( $post->ID );
									$author_id = $my_post->post_author;
