...
?>
	<form method="POST" action="" name="llms_start_quiz" enctype="multipart/form-data">
		...
		<input id="llms-user" name="llms-user_id" type="hidden" value="<?php echo $user_id; ?>"/>
	 	...
	 	<input id="llms_start_quiz" type="button" class="button" name="llms_start_quiz" value="<?php esc_html_e( 'Start Quiz', 'lifterlms' ); ?>" />
	 	...
	 	if ($lesson->get_next_lesson() && $quiz->is_passing_score( $user_id, $grade )) {
			$t = $lesson->get_next_lesson();
	 		?>
	 		<a href="<?php echo get_permalink( $lesson->get_next_lesson() );?>" class="button llms-next-lesson" style="text-decoration:none; display:inline;"><?php esc_html_e( 'Next Lesson','lifterlms' ); ?></a>
	 		<?php
	 	}
	 	?>
...
<?php
	...
	esc_html_e( '<p>You are not able take this quiz</p>', 'lifterlms' );
}
?>
</div>
