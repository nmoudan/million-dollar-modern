		...
		<?php } ?>
				<div class="col-md-4">
					<div class="llms-quiz-result-details">
						...
						<ul>
							<li>
								<h4><?php printf( esc_html__( 'Your Score: %d%%', 'lifterlms' ), $grade ); ?></h4>
								...
								<h6><?php printf( esc_html__( '%d / %d correct answers', 'lifterlms' ), $quiz->get_correct_answers_count( $user_id ), $quiz->get_question_count() ); ?></h6>
								<h6><?php printf( esc_html__( 'Date: <span class="llms_content_block">%s</span>', 'lifterlms' ), $start_date ); ?></h6>
								<h6><?php printf( esc_html__( 'Total time: %s', 'lifterlms' ), $time ); ?></h6>

								<?php if ($quiz->show_quiz_results()) { ?>
									<a class="view-summary"><?php esc_html_e( 'View Summary', 'lifterlms' ); ?></a>
								<?php } ?>
				...
				<div class="col-md-4">
					<div class="llms-quiz-result-details">
						...
						<ul>
							<li>
								<h4><?php printf( esc_html__( 'Best Score: %d%%', 'lifterlms' ), $best_grade ); ?></h4>
								<h5>
									<?php
									if ( $is_passing_score ) {
										apply_filters( 'lifterlms_quiz_passed','Passed' );
								...
								<h6><?php printf( esc_html__( '%d / %d correct answers', 'lifterlms' ), $quiz->get_correct_answers_count( $user_id, $best ), $quiz->get_question_count() ); ?></h6>
								<h6><?php printf( esc_html__( 'Date: <span class="llms_content_block">%s</span>', 'lifterlms' ), $start_date ); ?></h6>
								<h6><?php printf( esc_html__( 'Total time: %s', 'lifterlms' ), $best_time ); ?></h6>
							</li>
						</ul>
						<?php //endif; ?>
					</div>
...
<div class="llms-template-wrapper">
	<h3 class="llms-content-block">
		<?php printf( esc_html__( 'A score of  <span class="llms-content llms-pass-perc">%s%%</span> or more is required to pass this test.', 'lifterlms' ), $passing_percent ); ?>
	</h3>
</div>
<?php } ?>