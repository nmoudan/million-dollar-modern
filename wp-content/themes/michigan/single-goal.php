...
if(michigan_webnus_options::michigan_webnus_singlegoal_sidebar() == 'left'){ ?>
	<aside class="col-md-3 sidebar leftside">
		<?php if(is_active_sidebar('Left Sidebar')) dynamic_sidebar( 'Left Sidebar' ); ?>
	</aside>
<?php } ?>
<section class="<?php echo (michigan_webnus_options::michigan_webnus_singlegoal_sidebar()=='none')?'col-md-12':'col-md-9 cntt-w'?>">
...
<div class="row">
	...
	<div class="postmetadata">
		...
		<?php 
		if( isset($goal_features['category'])&& !isset($background) ){
	...
	<?php
		...
		if(isset($goal_features['sharing'])) {
		?>	
...
if(isset($goal_features['comment'])) {
 comments_template();
}
 ?>
</section>
...
<?php
if(michigan_webnus_options::michigan_webnus_singlegoal_sidebar() == 'right' ){ ?>
